...
import { executeTool, BUILTIN_TOOLS } from './src/ai/tools/registry.ts';
import { GuardrailsEngine } from './src/ai/safety/guardrails.ts';
...
app.post('/api/ai/reasoning/run', async (req, res) => {
  try {
    ...
    const activeTools = tools && tools.length > 0 ? tools : BUILTIN_TOOLS;
...
app.post('/api/ai/guardrails/check', (req, res) => {
  try {
    const { text } = req.body;
    const inspection = GuardrailsEngine.inspectInput(String(text || ''));
    res.json(inspection);
  } catch (err: unknown) {
    const msg = err instanceof Error ? err.message : String(err);
    res.status(500).json({ error: msg });
...
app.post('/api/ai/guardrails/verify-grounding', (req, res) => {
  try {
    const { answer, context } = req.body;
    const contextList = Array.isArray(context) ? context : [String(context || '')];
    const report = GuardrailsEngine.verifyGrounding(String(answer || ''), contextList);
    res.json(report);
  } catch (err: unknown) {
    const msg = err instanceof Error ? err.message : String(err);
    res.status(500).json({ error: msg });
