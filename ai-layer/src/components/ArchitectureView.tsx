...
export const ArchitectureView: React.FC = () => {
  ...
  const pillars = [
    {
      num: '01',
      title: isPt ? 'Abstração de Modelos' : 'Model Abstraction Interface',
      icon: Cpu,
      summary: isPt ? 'Contrato unificado desacoplado de provedores específicos.' : 'Unified contract decoupled from specific providers.',
      points: [
    ...
    {
      num: '02',
      title: isPt ? 'Engenharia de Contexto' : 'Context Engineering Layer',
      icon: Layers,
      summary: isPt ? 'Composição em camadas estritas sem sobrecarga de tokens.' : 'Strict multi-tier prompt composition avoiding monolithic blobs.',
      points: [
    ...
    {
      num: '03',
      title: isPt ? 'Subsistema de Memória' : 'Memory Subsystem (Short & Long-Term)',
      icon: Database,
      summary: isPt ? 'Janela deslizante curta e armazenamento semântico de longo prazo.' : 'Conversational state management and multi-criteria context retrieval.',
      points: [
    ...
    {
      num: '04',
      title: isPt ? 'Loop de Raciocínio Delimitado' : 'Bounded Reasoning Loop',
      icon: Zap,
      summary: isPt ? 'Decisões estruturadas e teto rígido de passos/orçamento.' : 'Deterministic structured decisions with hard step ceilings.',
      points: [
    ...
    {
      num: '05',
      title: isPt ? 'Roteamento Dinâmico de Modelos' : 'Dynamic Model Routing',
      icon: Route,
      summary: isPt ? 'Otimização de custo e latência baseada em complexidade.' : 'Cost and latency optimization based on task complexity.',
      points: [
    ...
    {
      num: '06',
      title: isPt ? 'Escudo de Segurança & Guardrails' : 'Safety Shield & Guardrails',
      icon: ShieldAlert,
      summary: isPt ? 'Detecção de Jailbreak, mascaramento de PII/Segredos e aterramento.' : 'Prompt injection defense, PII redaction, and hallucination defense.',
      points: [
        'Prompt Injection Engine: regex & semantic signature detection of bypass attempts',
        'Data Privacy & PII: auto-redaction of CPF, credit cards, and GitHub/API tokens',
        'Grounding Verifier: lexical overlap and factual grounding scoring vs. trusted docs',
      ],
    },
    {
      num: '07',
      title: isPt ? 'Registro de Ferramentas em Tempo Real' : 'Live Tool Registry',
      icon: Wrench,
      summary: isPt ? 'Execução real e segura de utilitários de sistema e cálculo.' : 'Safe, deterministic execution of live tools and arithmetic engines.',
      points: [
        'Built-in sandbox: calculator, current ISO timestamp, knowledge search, text analyzer, metrics',
        'Strict JSON schema validation for parameters prior to execution',
        'Deterministic error handling with observation logging for agent reflection',
      ],
    },
    {
      num: '08',
      title: isPt ? 'Avaliação & Benchmarks' : 'Evaluation & Benchmarks',
      icon: ShieldCheck,
      summary: isPt ? 'Avaliação quantitativa em 8 dimensões de qualidade.' : 'Quantitative evaluation across 8 dimensions.',
      points: [
